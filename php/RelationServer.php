 ...
 * @package service
 * @author Anyon <xiongxianhua@yeah.net>
...
EOT;
    ...
    public function __construct() {
        $this->_data = json_decode($this->json_data, true);
        $this->_filter = json_decode($this->json_filter, true);
        //结局json_decode 不支持 反斜杠的问题,手动添加带有反斜杠的数据
        $this->_filter[] = [
            "exp" => "/(.+)?\[(.+)\|(.+)\](.+)?/",
            "str" => "$1$2$4#$1$3$4"
        ];
    ...
    public function unique($arr) {
        ...
        $hash = [];
        $item;
        for ($i = 0; ($item = $arr[$i]) != null; $i++) {
            $temp = preg_replace("/[ol](?=s|b)/", "x", $item);
            $temp = preg_replace("/&[ol]/", "", $item); //对特殊语法标识相互包含的行为去重
    ...
    function getSelectors($str) {
        ...
        $lists = explode("的", $str); // split("的", $str);
        $result = [];      //所有可能性
        $match = true;
        while (count($lists)) {
            $name = array_shift($lists);   //当前匹配词
            $arr = [];      //当前匹配词可能性
            $has = false;
            //$_data = json_decode($this->_data, true);
            foreach ($this->_data as $key => $value) {
                if (in_array($name, $value)) {  //是否存在该关系
                    if ($key || !count($lists)) {  //对‘我’的优化
                        array_push($arr, $key);
                    }
                    $has = true;
                }
            ...
            if (count($result)) {     //当前匹配词与之前可能性组合
                $res = [];
                for ($i = 0; $i < count($result); $i++) {
                    for ($j = 0; $j < count($arr); $j++) {
                        array_push($res, $result[$i] . ',' . $arr[$j]);
                    }
                }
                $result = $res;
            } else {
    ...
    public function getId($selector, &$result, &$hash) {
        //echo var_export($hash) . "==sels<br>";
        $s = '';
        if (!$hash[$selector]) {
            $hash[$selector] = true;
            $status = true;
            do {
                $s = $selector;
                //$_filter = json_decode($this->_filter, true);
                foreach ($this->_filter as $key => $item) {
                    //var item = _filter[i];
                    // console.log('filter#',item['exp'],selector);
                    $selector = preg_replace("{$item['exp']}", $item['str'], $selector);
                    //echo $key."===".$selector."==".$item['exp']."===".$item['str']."==sel==index<br>";
                    //echo $selector . "===sel={$item['exp']}" . "<br>";
                    if (strpos($selector, "#") !== false) {
                        $arr = explode("#", $selector);
                        for ($j = 0; $j < count($arr); $j++) {
                            $this->getId($arr[$j], $result, $hash);
                        }
                        $status = false;
                        break;
                    }
            ...
            if ($status) {
                ...
                $selector = substr(preg_replace("/,[01]/", '', $selector), 1);
    ...
    function selector2id($selector, $sex) {
        ...
        $this->getId($selector, $result, $hash);
        ...
        $getData = function($d) {
            ...
            foreach ($this->_data as $key => $value) {
                if (preg_replace("/&[olx]/", "", $key) == $d) {
                    array_push($res, $this->_data[$key][0]);
                }
            }
            return $res;
        };
        if ($this->_data[$id]) {  //直接匹配称呼
            array_push($items, $this->_data[$id][0]);
        } else {
            $items = $getData($id);
            if (!count($items)) {  //忽略年龄条件查找
...
EOT;
        ...
        if (strpos($id, "&o") !== false) {
            $age = '&l';
        } else if (strpos($id, "&1") !== false) {
            $age = '&o';
        }
        if ($id) {
            $id = preg_replace("/&[ol]/", "", $id);
            $sex = $sex ? 1 : 0;  //逆转运算自身性别必须确定                        
            $temp = preg_replace("/,[fhs]|,[olx]b/", ',1', (',' . $sex . ',' . $id));
            $sid = preg_replace("/,[mwd]|,[olx]s/", ',0', $temp);
            $sid = substr($sid, 0, strrpos($sid, ","));
            $id_arr = array_reverse(explode(",", $id));
            $sid_arr = array_reverse(explode(",", $sid));
            $arr = [];
            for ($i = 0; $i < count($id_arr); $i++) {
                array_push($arr, $hash[$id_arr[$i]][$sid_arr[$i]]);
            }
    ...
    function getChainById($id) {
        ...
        $items = [];
        for ($i = 0; $i < count($arr); $i++) {
            $key = preg_replace("/&[ol]/", '', $arr[$i]);
            array_push($items, $this->_data[$key][0]);
        }
        return explode("的", $items);
    }

    public function relactionShip($parameter) {
        $options = [
            "text" => '',
            "sex" => -1,
        ...
        for ($i = 0; $i < count($selectors); $i++) {  //遍历所有可能性
            ...
            for ($j = 0; $j < count($ids); $j++) {
                ...
                if ($options['type'] == 'chain') {
                    ...
                    if ($options['reverse']) {
                        $id = $this->reverseId($id, $options['sex']);
                    }
                    $items = $this->getDataById($id);
                    if (count($items)) {
                        $result = array_merge($result, $items);
                    } else if (strpos($id, "w") == 0 || strpos($id, "h") == 0) {  //找不到关系，随爱人叫
